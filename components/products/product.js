...
// firebase
    ...
    // db.collection('san_pham').get().then(function(querySnapshort){
    //     let _records = [];
    //     querySnapshort.forEach(function (doc){
    //         //console.log(doc.data(),doc.id);
    //         let item = {_id: doc.id, ...doc.data()}
    //         _records.push(item);
    //     })
    //     $scope.products = _records;
    //     $scope.$apply();
    ...
    // dùng http service
    $http.get('https://623ede5fdf20a75d53cc47e4.mockapi.io/product')
    ...
    $scope.deleteProduct = function (id) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $http({
                        url: `https://623ede5fdf20a75d53cc47e4.mockapi.io/product/${id}`,
                        method: "DELETE",
                    })
                        .then(function (response) {
...
app.controller('productFormController', function ($scope, $rootScope, $location,
    ...
    if ($scope.productId) {
        $http.get(`https://623ede5fdf20a75d53cc47e4.mockapi.io/product/${$scope.productId}`)
            .then(function (response) {
    ...
    $scope.submitForm = function () {
        ...
        if ($scope.productId) {
            ...
            $http({
                url: `https://623ede5fdf20a75d53cc47e4.mockapi.io/product`,
                method: "POST",
                data: { name, price, quantity, description, image }
            })
                .then(function (response) {
